...
namespace BVNetwork.NotFound.Core.Data
{
    ...
    public class DataStoreEventHandlerHook : EPiServer.PlugIn.PlugInAttribute
    {
        ...
        private static readonly Guid DataStoreUpdateEventId = new Guid("{26A1CA35-1CBD-44a7-8243-5E80D79F3F26}");
        private static readonly Guid DataStoreUpdateRaiserId = new Guid("{6180555A-7A0E-4485-B1B1-44BF6E4D4A0D}");
        ...
        public static void Start()
        {
            try
            {
                if (Event.EventsEnabled)
                {
                    ...
                    _log.Debug(
                        "Domain ID: '{0}', Friendly Name: '{1}', Basedir: '{2}', Thread: '{3}'",
                        AppDomain.CurrentDomain.Id.ToString(),
                        AppDomain.CurrentDomain.FriendlyName,
                        AppDomain.CurrentDomain.BaseDirectory,
                        System.Threading.Thread.CurrentThread.ManagedThreadId.ToString());
                    // Listen to events
                    var dataStoreInvalidationEvent = Event.Get(DataStoreUpdateEventId);
        ...
        public static void DataStoreUpdated()
        {
            ...
            var dataStoreInvalidateEvent = Event.Get(DataStoreUpdateEventId);
            // Raise event
